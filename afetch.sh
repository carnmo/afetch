...
stats["net"]="printf \"%s TCP, %s UDP connections\n\" \"$(tail -n +2 /proc/net/tcp | wc -l)\" \"$(tail -n +2 /proc/net/udp | wc -l)\""
stats["os"]="cat /etc/os-release | grep PRETTY_NAME | cut -d'\"' -f 2"
stats["packages"]="pacman -Q | wc -l"
stats["processes"]="ps ax | wc -l"
stats["ram"]="printf \"%s total, %s free\n\" \"$(grep MemTotal /proc/meminfo | cut -d ':' -f 2 | xargs)\" \"$(grep MemFree /proc/meminfo | cut -d ':' -f 2 | xargs)\""
